...
def stud1():
    ...
    for i in data2:
        for j in i:
            demo2=j
    if(str(demo1)==ssid):
        return render_template('links.html',Dat1=dem2,Data=demo2)
    else:
...
def proct1():
    ...
    for i in dat2:
        for j in i:
            dem3 = j
    cursor.execute("SELECT S.Sid,S.Fname,S.Lname,S.Branch,S.DOB,S.Phone,S.Address,S.Problem from Student S,Proctor P WHERE S.Pid=P.Pid AND P.Pid=%s " % ppid)
